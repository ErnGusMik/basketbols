...
export default function NewTournamentSend() {
  ...
  document.title = lang
    ? "Create | New tournament | Gandriz NBA"
    : "Izveidot | Jauns turnīrs | Gandrīz NBA";
  ...
  const createTournament = async () => {
    ...
    if (response.error && !sentOnce) {
      setSentOnce(true);
      mainText.current.innerText = lang ? "Error!" : "Kļūda!";
      ...
      sendingNotesSub.current.innerText =
        (lang
          ? "Enter the necessary information and try again (the rest will be saved). (Redirecting in "
    ...
    console.log("[INFO] " + Date.now() + ": Tournament successfully created");
  ...
  const sendPlayers = async (teamIDs, tournamentID) => {
    ...
    if (!teamIDs) return console.log("Error: teamIDs not received from server");
  ...
  const final = async () => {
    ...
    sendingNotes.current.innerText = lang ? "Nearly there!" : "Gandrīz gatavs!";

    await sendReferees(tournamentID);

    setTimeout(() => {
      sendingNotes.current.innerText = lang
        ? "This is taking a bit longer than usual..."
        : "Šis aizņem mazliet vairāk laika nekā parasti...";
      sendingNotesSub.current.innerText = lang
        ? "Please don't refresh the page!"
        : "Lūdzu, neatsvaidziniet lapu!";
    }, 5000);
    ...
    <div className="new-tournament-send__container">
      <div className="new-tournament-send">
        <p className="sending" ref={mainText}>
          {lang ? "Creating your tournament..." : "Veidojam jūsu turnīru..."}
        </p>
